...
namespace meng_pungi_sticks
{
    ...
    internal class meng_pungi_sticks_poison_Building_TrapDamager : Building_TrapDamager
	{
		...
		private void SpringSubImpl(Pawn hitPawn)
		{
			...
			float num = this.GetStatValue(StatDefOf.TrapMeleeDamage, true) * meng_pungi_sticks_poison_Building_TrapDamager.DamageRandomFactorRange.RandomInRange / meng_pungi_sticks_poison_Building_TrapDamager.DamageCount;
			float armorPenetration = num * 0.015f;
			int num2 = 0;
			while ((float)num2 < meng_pungi_sticks_poison_Building_TrapDamager.DamageCount)
			{
				...
				if (num2 == 0)
				{
					BattleLogEntry_DamageTaken battleLogEntry_DamageTaken = new BattleLogEntry_DamageTaken(hitPawn, meng_pungi_sticks_poison_Building_TrapDamager.MengRulePackDefOf.DamageEvent_TrapMengPunjiSticks, null);
		...
		private void RandomPoison(Pawn hitPawn)
		{
			if (Rand.Value <= 0.25f)
			{
				Messages.Message(TranslatorFormattedStringExtensions.Translate("meng_punji_sticks_PoisonSuccess", hitPawn.Label), MessageTypeDefOf.PositiveEvent, false);
				Hediff hediff;
				if (hitPawn == null)
				{
					hediff = null;
				...
				else
				{
					...
					else
					{
						...
						hediff = ((hediffSet != null) ? hediffSet.GetFirstHediffOfDef(HediffDefOf.ToxicBuildup, false) : null);
					}
				}
